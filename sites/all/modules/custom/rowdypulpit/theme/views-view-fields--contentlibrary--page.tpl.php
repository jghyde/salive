...
$content = '';
$attributes = array();
if (url_is_external($fields['field_contrib_url']->content)) {
  $attributes = array('attributes' => array('target' => '_blank', 'class' => array('read-more')));
} 
$contrib_url = l('Read more', $fields['field_contrib_url']->content, $attributes);
$cl = '';
global $user;
if ($user->uid == 1) {
...
switch ($fields['field_type_of_posting']->content) {
  case 'short':
    $content = l($fields['title']->content, $fields['field_contrib_url']->content, $attributes);
  break;
  case 'long':
  ...
  break;
  case 'related':
    $content = '<div class="rowdy-related">Related: ' .  l($fields['title']->content, $fields['field_contrib_url']->content) . '.</div>';
    $content .= $cl;
  break;
  default:
    if (empty($fields['field_contrib_img']->content)) {
      ...
      $img = _rowdypulpit_format_image($fields['field_contrib_img']->content, 'contentlibrary_top_story', $fields['title']->content);
    }
    
    $content =  '<h2 class="content-title">' . l($fields['title']->content, $fields['field_contrib_url']->content, $attributes) . '</h2>' . $cl;    
    if (!empty($img)): $content .= $img; endif;
